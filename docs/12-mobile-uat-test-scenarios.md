...
| `UAT-MV2-21` | Penurunan Qty > 5 memicu Void Sheet (butir 5) | 1. Tambahkan 8 pcs produk ke keranjang.<br>2. Tekan tombol minus enam kali berturut-turut. | 8 pcs → 2 pcs | Setelah penurunan gabungan melewati ambang 5, aplikasi menolak menurunkan diam-diam dan menuntut `reason_code` + catatan.<br><br>⚠️ **Diketahui GAGAL pada build saat ini** — lihat catatan §3 butir 5. Skenario ini adalah gerbang rilis untuk butir 5 di mobile. | Offline | `[ ] PASS` |
...
4. **Butir 5 (Strict Qty Audit) BELUM terpasang di UI keranjang mobile.** `CartCubit.canDecrementTo()` sudah ada dan benar, tetapi tombol minus pada `cart_panel.dart` memanggil `onDecrement` secara langsung tanpa melewatinya — sehingga penurunan kuantitas berapa pun lolos tanpa Void Sheet. Versi Web sudah terpasang penuh. `UAT-MV2-21` akan **GAGAL** sampai ini diperbaiki, dan kegagalan itu memang yang seharusnya terjadi.
