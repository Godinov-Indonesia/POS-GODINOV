...
func SetupRouter(
	...
	posDevice := func(handler http.HandlerFunc) http.HandlerFunc {
		return deviceMiddleware(middleware.RequireDeviceScope(token.ScopePOS)(handler))
	}
	opnameDevice := func(handler http.HandlerFunc) http.HandlerFunc {
		return deviceMiddleware(middleware.RequireDeviceScope(token.ScopeOpname)(handler))
	}
